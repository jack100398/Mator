...
@section('content')
    <div id="app">
        ...
        <a href="{{ route('createProductTypePage') }}">
        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                <select class="form-control" onchange="location = this.value;">
                    <option value="{{ route('product-type') }}" selected>全選
                    </option>
                    <option value="{{ route('product-type', ['site' => 'zh']) }}" {{ $site === 'zh' ? 'selected' : '' }}>中文站
                    </option>
                    <option value="{{ route('product-type', ['site' => 'en']) }}" {{ $site === 'en' ? 'selected' : '' }}>英文站
                    </option>
                </select>
            </div>
        </div>
